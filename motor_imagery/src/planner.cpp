...
public:
    Planner()
    {
        ...
        // Definire la velocità target e il tempo di campionamento
        const double TARGET_VELOCITY = 10.0;   // cm/s
        const double SAMPLING_TIME = 0.01;     // secondi
        const double BASE_MOVEMENT = TARGET_VELOCITY * SAMPLING_TIME; // 0.1 cm per step

        // Iniziamo il ciclo di pubblicazione a 100 Hz
        while (ros::ok())
        {
            // Calcolo della nuova posizione solo se c'è movimento
            finalPoint = CURRENT_POSITION_ + extractedNumber_;
          ...
          // Aggiornamento del messaggio waypoint
            ...
            WAYPOINT_MSG_.linearVelocity.y = 0.1;
